...
<div id="rezgo" class="wrp_book">
	<div id="panel_full">
	    	...
	    	<?
	          ...
	          <div class="cart_name">
	          	<ol class="cart_detail">
	            	<li class="cart_tour_name">
	          			<h1 class="tour_title">
	          				<a href="<?=$site->base?>/details/<?=$order->com?>/<?=$site->seoEncode($order->item)?>">
	          					<?=$order->item?><span class="cart_option_name">&nbsp;(<?=$order->option?>)</span>
	          				</a>
	          			</h1>
	              </li>
	          ...
	          <div class="cart_cost">
	          	<ol class="cart_detail">
	                  ...
	                  <li class="cart_edit">
	                  	<input class="cart_edit" type="submit" onclick="document.location.href='<?=$site->base?>/details/<?=$order->com?>/<?=$site->seoEncode($order->item)?>/<?=$order->uid?>/<?=date("Y-m-d", $order->date)?><?=$pax_edit_string?>';" value="Make Changes">
	                  </li>
      ...
      <div class="cart_action">
		...
		<div id="booking_link">
			<a href="javascript:void(0);" onclick="jQuery('#booking_link_val').toggle('fade');">share this order</a>
