	...
	<? foreach( $site->getTours('t=uid&q='.$order->uid.'&d='.date("Y-m-d", $order->date).$pax_string) as $item ) { ?>
	  ...
	  <div class="tour_info">
		  ...
		  <ol class="tour_receipt">
		  	<li class="info"><label>You are booking</label><span><?=$item->item?><br /><?=$item->option?></span></li>
...
<script>
	...
	function stepBack() {
		...
		<? if($site->getPaymentMethods('PayPal')) { ?>
			...
			$('#payment_paypal').html('<br><br><img src="<?=$site->path?>/images/booking_load.gif">');
			<? } ?>
	...
	function paypalConfirm(token, payerid, name, email) {
		...
		$('#payment_paypal').html(string);
		$('#payment_paypal').fadeIn();
		
		$('#paypal_token').val(token);
