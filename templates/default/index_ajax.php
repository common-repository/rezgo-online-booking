...
<div id="left_panel"-->
    ...
    <h1 class="tour_title"><a href="<?=$site->base?>/details/<?=$item->com?>/<?=$site->seoEncode($item->item)?>"><?=$item->item?></a></h1>
    <div class="location">
    	<span>Location:&nbsp;</span>
    	<?
    		unset($loc);
    ...
    <? } ?>
    <div class="btn_details"><a href="<?=$site->base?>/details/<?=$item->com?>/<?=$site->seoEncode($item->item)?>">details</a></div>
		     			...
		     			<div class="overlay" id="content_<?=$day->id?>">
								...
								<div id="rezgo_popup_book">
								    ...
								    <h3 class="popup_title"><?=date("F d, Y", $day->date)?></h3>
										<h1 class="popup_title"><?=$item->item?></h1>
								  </div>
								  <div class="wrp">
								  	<div class="modal_titles">
										...
										<div id="cal_page_<?=$day->id?>_<?=$day->date?>_<?=$p++?>">
											<? foreach( $day->items as $option ): ?>
										  	...
										  	<div class="result_01"><?=$option->item?></div><div class="result_02"><? if($option->availability == 0) { ?>full<? } elseif($option->availability == '9999' OR $option->hide_availability == 1) { ?>available<? } else { ?><?=$option->availability?><? } ?></div><div class="result_03"><span><a href="<?=$site->base?>/details/<?=$item->com?>/<?=$site->seoEncode($item->item)?>/<?=$option->uid?>/<?=date("Y-m-d", $day->date)?>">book now</a></span></div>
